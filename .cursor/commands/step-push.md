...
1. Read and follow [`.cursor/skills/step-ship/SKILL.md`](../skills/step-ship/SKILL.md) **Push mode** only.
2. **Do not** run `git commit` unless the user also explicitly asked to commit in the same turn.
3. If the branch is not a `step/…` branch or the tree is dirty, follow `step-ship/SKILL.md` stop rules and **ask** — do not guess.
